...
// 实数向量减法
void runRealSub(ofstream& res, Vector<double>& v1, Vector<double>& v2) {
    cout << "\nVector 1:\n" << v1;
    cout << "\nVector 2:\n" << v2 << "\n";
    res << "Vector 1:\n" << v1 << "\n";
    res << "Vector 2:\n" << v2 << "\n";
...
// 复数向量减法
void runComplexSub(ofstream& res, Vector<Complex>& v1, Vector<Complex>& v2) {
    cout << "\nVector 1:\n" << v1;
    cout << "\nVector 2:\n" << v2 << "\n";
    res << "Vector 1:\n" << v1 << "\n";
    res << "Vector 2:\n" << v2 << "\n";
...
int main(int argc, char* argv[]) {
    ofstream resultFile("vector_sub_result.txt");
    cout << fixed << setprecision(4);
    resultFile << fixed << setprecision(4);

    cout << "===== AUTO Vector Subtraction (REAL + COMPLEX) =====\n\n";
    resultFile << "===== AUTO Vector Subtraction (REAL + COMPLEX) =====\n\n";
    ...
    if (argc >= 2) {
        ...
        if (isComp) {
            ...
            if (file.is_open() && loadComplexVector(file, v1) && loadComplexVector(file, v2)) {
                useFile = true;
                runComplexSub(resultFile, v1, v2);
            }
        } else {
    ...
    cout << "Result saved to vector_sub_result.txt\n";
    resultFile.close();
    return 0;
}