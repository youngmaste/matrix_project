...
// 实数向量点积
void runRealDot(ofstream& res, Vector<double>& v1, Vector<double>& v2) {
    cout << "\nVector 1:\n" << v1;
    cout << "\nVector 2:\n" << v2 << "\n";
    res << "Vector 1:\n" << v1 << "\n";
    res << "Vector 2:\n" << v2 << "\n";
...
// 复数向量点积
void runComplexDot(ofstream& res, Vector<Complex>& v1, Vector<Complex>& v2) {
    cout << "\nVector 1:\n" << v1;
    cout << "\nVector 2:\n" << v2 << "\n";
    res << "Vector 1:\n" << v1 << "\n";
    res << "Vector 2:\n" << v2 << "\n";
...
int main(int argc, char* argv[]) {
    ofstream resultFile("vector_dot_result.txt");
    cout << fixed << setprecision(4);
    resultFile << fixed << setprecision(4);

    cout << "===== AUTO Vector Dot Product (REAL + COMPLEX) =====\n\n";
    resultFile << "===== AUTO Vector Dot Product (REAL + COMPLEX) =====\n\n";
    ...
    if (argc >= 2) {
        ...
        if (isComp) {
            ...
            cout << "Using: v1 · v2 = sum(v1[i] * conj(v2[i]))\n\n";
            resultFile << "Detected: COMPLEX vectors\n";
            resultFile << "Using: v1 · v2 = sum(v1[i] * conj(v2[i]))\n\n";

            Vector<Complex> v1, v2;
            if (file.is_open() && loadComplexVector(file, v1) && loadComplexVector(file, v2)) {
                useFile = true;
                runComplexDot(resultFile, v1, v2);
            }
        } else {
            cout << "Detected: REAL vectors\n\n";
            resultFile << "Detected: REAL vectors\n\n";

            Vector<double> v1, v2;
            if (file.is_open() && loadRealVector(file, v1) && loadRealVector(file, v2)) {
                useFile = true;
                runRealDot(resultFile, v1, v2);
            }
        }
    ...
    cout << "\nResult saved to vector_dot_result.txt\n";
    resultFile.close();
    return 0;
}