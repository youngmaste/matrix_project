...
// 实数矩阵条件数计算
void runRealCond(ofstream& res, Matrix<double>& A) {
    cout << "\nMatrix A:\n" << A;
    res << "Matrix A:\n" << A << "\n";
    ...
    if (cond == HUGE_VAL) {
        cout << "Condition Number = infinity\n";
        res << "Condition Number = infinity\n";
    } else {
        cout << "Condition Number = " << cond << "\n";
        res << "Condition Number = " << cond << "\n";
    }
}

// 复数矩阵条件数计算
void runComplexCond(ofstream& res, Matrix<Complex>& A) {
    cout << "\nMatrix A:\n" << A;
    res << "Matrix A:\n" << A << "\n";
...
int main(int argc, char* argv[]) {
    ofstream resultFile("matrix_cond_result.txt");
    cout << fixed << setprecision(4);
    resultFile << fixed << setprecision(4);

    cout << "===== AUTO Matrix Condition Number (REAL + COMPLEX) =====\n\n";
    resultFile << "===== AUTO Matrix Condition Number =====\n\n";
    ...
    if (argc >= 2) {
        ...
        if (isComp) {
            ...
            if (file.is_open() && loadRealMatrix(file, A)) {
                useFile = true;
                runRealCond(resultFile, A);
            }
        }

        if (!useFile) {
            cout << "File load failed! Switch to manual input.\n\n";
            resultFile << "File load failed, use manual input.\n\n";
        }
        file.close();
    }
    ...
    cout << "\nResult saved to matrix_cond_result.txt\n";
    resultFile.close();
    return 0;
}